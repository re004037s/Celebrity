module ApplicationHelper
  ...
  def movie_comp?
    if current_user.free_engineer_user && current_user.venture_user
      if Feedback.where(user_id: current_user).count == Movie.where(movie_category_id: MovieCategory.where(must_view: true).ids).count
        return true
      else
        return false
      end
    elsif current_user.free_engineer_user
      if Feedback.where(user_id: current_user).count == Movie.where(movie_category_id: MovieCategory.where(must_view: true).where(free_engineer_movie: true).ids).count
        return true
      else
        return false
      end
    elsif current_user.venture_user
      if Feedback.where(user_id: current_user).count == Movie.where(movie_category_id: MovieCategory.where(must_view: true).where(venture_movie: true).ids).count
        return true
      else
        return false
      end
